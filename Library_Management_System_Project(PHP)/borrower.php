...
<form class="form" method="get" action="borrower.php">
	...
	<?php
		...
		{	
			$insert_query="Insert into BORROWER (Ssn, Fname, Lname, Email, Address, City, State, Phone) values('".$data1."','".$data2."','".$data3."','".$data8."' ,'".$data4."','".$data5."','".$data6."','".$data7."')";
			$result=mysqli_query($con,$insert_query);
			  $queryCount="Select Card_no from  borrower where Ssn='".$data1."'";
			$resultCount=mysqli_query($con,$queryCount);
			$row=mysqli_fetch_array($resultCount);
			$count=$row['Card_no'];
			if($result)
				{
				echo '<script type="text/javascript">alert("Borrower added successfully. Your Card Number: " + "'.$count.'");</script>';
			?>
